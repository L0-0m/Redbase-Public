...
#define START_GLOBAL_ERR    (-501) 
#define END_GLOBAL_ERR      (-600) 
...
#define START_GLOBAL_WARN   501
#define END_GLOBAL_WARN     600
...
//
enum AttrType {
    INT,        // first ele for bounds checking
    FLOAT,      // 从中间插入新类型不会破坏边界检查
    STRING      // last ele for bounds checking
};
...
//
enum CompOp {
    NO_OP,                                      // no comparison, upper bounds
    EQ_OP, NE_OP, LT_OP, GT_OP, LE_OP,          // binary atomic operators, 从中间插入新类型不会破坏边界检查
    GE_OP                                       // lower bounds
};
